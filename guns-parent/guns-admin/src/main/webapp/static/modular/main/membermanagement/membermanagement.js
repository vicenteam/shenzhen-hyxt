...
Membermanagement.initColumn = function () {
    return [
        ...
        {title: '总消费', field: 'price', visible: false, align: 'center', valign: 'middle'},
...
Membermanagement.deptjifencishuzengsong = function () {
      var depts= $("#deptid").html();
        layer.confirm('<div>门店：<select id="deptId">'+depts+'</select></div><div>赠送可签到积分次数数量：<input type="text" style="width: 50px"value="0" id="jifenNum"></div>', {btn: ['确定', '取消']}, function () {
            layer.closeAll('dialog');
            var ajax = new $ax(Feng.ctxPath + "/membermanagement/deptjifencishuzengsong", function (data) {
                Feng.success("赠送成功!");
                Membermanagement.table.refresh();
            }, function (data) {
                Feng.error("赠送失败!" + data.responseJSON.message + "!");
            });
            ajax.set("id", $("#deptId").val());
            ajax.set("jifenNum", $("#jifenNum").val());
            ajax.start();
        });
};
